...
### 12.1 추가 (2026-08-05) — 위 2번째 불릿의 **실측된 구체형**

위 *"사용자가 개념을 이해 못 하면 상위 개념·장단점 설명"* 은 방향은 맞지만
형식이 비어 있었다. 사용자가 **효과가 있었다고 직접 확인한** 형식이 측정됐다.
아래는 기존 불릿을 대체하지 않고 그 실행형을 덧붙인 것이다.

**요청 원문(사용자):** *"시각적으로 연상이 잘 되도록 [Input_data → Task →
Output_data, data_shape=round, task_shape=straightforward] 해부도를 설명하듯,
가장 추상적이고 단순한 그래프 표현에서 시작해 사용자가 더 보고 싶은 부품을
확대하면 더 구체적인 그래프가 드러나고, 이 흐름을 반복."*

**확인:** *"실행도 잘 되었고 이해도 된다."*

지켜야 하는 것 5개:

1. **L0을 먼저, 그리고 L0에서 논증이 끝나게.** 가장 추상적인 그래프 한 장에
   결론이 이미 보여야 한다. 상세는 확대의 결과일 뿐 이해의 전제가 아니다
2. **`( 데이터 ) → [ 과업 ] → ( 데이터 )` 표기.** 데이터는 원형(pill),
   과업은 직사각. 사용자가 지정한 규약이므로 임의로 바꾸지 않는다
3. **핵심 결함은 시각적 부재로 표현한다.** 이번 사례의 성공 요인 — 설명해야
   할 것이 "검사가 없다"였고, 과업 자리를 **점선 빈 칸**으로 비워 두자
   문장 없이 전달됐다. 없는 것을 말로 논증하지 말고 자리를 비워 보여라
4. **확대는 사용자가 고른다.** 레이어를 전부 펼쳐 놓지 말고 접어 두고,
   L0의 부품을 눌러 해당 레이어로 들어가게 한다
5. **한계를 같은 화면 안에.** n=1, 선택 편향 등을 마지막 블록으로 붙인다

**전달 매체:** Claude Artifact(HTML). 발행본이 계정 문제로 안 열릴 수 있으므로
`doctype`+`<head>`를 직접 감싼 **자립형 로컬 사본을 함께** 만들어 여는 것이
안전하다 — 이번에 실제로 발행본이 `Page not found`였고 로컬 사본이 살렸다.

**실측 사례:** `concept-gate-owl-wt/experiments/2026-08-04_owl_entailment_contract_shape/OPERATIONS_LOG.md`
§2026-08-05 판정의 "가드 음성-테스트 메타 테스트가 필요하다"를 L0~L6
해부도로 설명한 건.

**주의 — 이 항목의 범위:** 이것은 *설명 형식*에 대한 선호다. §12 첫 불릿의
"매 결정마다 질문, 실행 후 보고 금지"를 완화하지 않는다. 해부도는 결정을
대신하는 것이 아니라 결정을 이해시키는 수단이다.
