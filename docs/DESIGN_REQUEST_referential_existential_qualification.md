...
- **FOLIO 상수 사용은 잡음이 있다.** 116건 중에는 지시 표현이 아닌 것도
  섞인다 — `music`(20p2, 물질명사) · `on`(598p4, "Ontario"의 파싱 잔여로
  보인다). 116건을 부류별로 분류하지 않았다. "14%가 상수를 쓴다"는 사실이고
  "14%가 지시 표현이다"는 주장이 아니다.
- **PMB 쪽 전수는 20건이다**(in-N 15 + control 5). PMB gold 전체(12,053건)에
  대한 비율은 재지 않았다.
