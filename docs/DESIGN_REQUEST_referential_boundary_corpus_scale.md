...
**그리고 그 51%조차 재료의 성질이 아니다**(2차 정정, D-34 수신 검증 §B.6).
목록 구성을 바꿔 같은 15,810개에 재실측하면: 양화만 **2%** · +인칭·지시
**41%** · +소유 **51%** · +재귀 51%. **2%에서 51%까지 움직인다.**

그러므로 보고할 수 있는 것은 특정 비율이 아니라 이것이다 — **닫힌 목록의
유형화 범위가 목록 구성에 25배 민감하다.** 방향("경계를 목록으로 그릴 수
없다")은 오히려 강해진다: 대표 수치가 그만큼 흔들리는 유형을 의미론적 경계라고
주장할 근거는 없다.
