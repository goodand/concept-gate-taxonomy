...
| **Q38** | **판정 도착(D-38)** — 인증 프로파일 `required` 에 새 의무(`claim.evidence_provenance`)를 추가하는 절차. 계기는 L2 뿌리 공백 실측: 어휘 결박이 **호출자가 준 evidence 안에서만** 검사하므로 원문에 없는 문장을 넣어도 `PASS`·`RULE_CHECKED` 였다(원문 "포함하지 **않는다**" + 날조 "포함한다"). 선행 확인으로 **5건은 이미 답이 있어 문항에서 뺐다**(D4 = 소급 미충족은 결함이 아니라 정직한 표시 · D-24 §9 amendment 절차 · D-32 Q32.4 = 새 버전+amendment 둘 다 · 두 금지는 measurand 경계 전용이라 무관(실측) · 인증은 저장 안 되는 view 라 "발급된 인증서의 지위" 문항 자체가 부정확). **새 질문 셋**: ㄱ `_v0` 제자리 확장 vs `_v1` 신설(선례가 서로 다른 답을 준다) · ㄴ **서명 본체에 `profile` 이 없다** — profile 개정 이력의 담지자가 없음(`CERTIFICATE_SCHEMA` v0→v1 선례 있음) · ㄷ 배포 MCP 호출 이력이 실험 dispatch 에 상응하는가(= 이미 "관측 후"인가) | [[DESIGN_REQUEST_certification_profile_amendment|Q38]] | [[DESIGN_DECISION_certification_profile_amendment]] |
| D-38 | **프로파일 개정은 `required += 1` 이 아니라 “어떤 계약으로 인증했는가”의 역사성 문제다.** ㄱ **NEW_PROFILE_IDENTITY_PREFERRED** — 제자리 확장은 논리적으로 가능하나 계약 안전하지 않고, `_v1` 은 논리적 필연이 아니라 **감사 계약 요구**(D-32 선례가 이 의미로 정당화된다) · ㄴ **서명 본체에 profile commitment 필요** — 없으면 동일 `results[]` 의 두 문서를 verifier 가 구별 못 한다. 허용 형태 `profile_id` 또는 `+required_hash`, **서명 payload shape 이 바뀌면 schema bump 필요**(profile version ⊥ certificate schema version) · ㄷ **MCP 호출 ≠ 실험 관측** — 제품 계약 사용은 별개 축이나 **별도 backward-compatibility 규칙이 필요하다는 근거**는 된다 · 금지 3종 unchanged. **우리 수신 검증**: V1·V2 판정 집합 128 전수로 단조성 확증(정방향 반례 0) · **V3 이 판정보다 강한 결과** — 두 profile 의 인증서 payload 가 바이트 동일(sha256 동일)이고 그 payload 엔 signature 조차 없다 · V6 이 우리 상신서 표현을 정정(인증 경로는 배포돼 있다, MCP tool 13종) · **V8 미결** — 판정이 명시한 조건(“v0 의 역사적 의미 보존”)의 성립 여부는 우리 몫이고 재료가 양방향(저장 인증서 0건 vs 배포된 tool 13종) | [[DESIGN_REQUEST_certification_profile_amendment|Q38]] | [[DESIGN_DECISION_certification_profile_amendment]] |
