...
| `conceptgate/cg_mrs_reader.py` | `MANUAL_TOOL` | `docs/RESEARCH_RESULT_mrs_redwoods_round3.md:1272` — 이 파서로 **37,060건 파싱 성공**(거부 6건 = 0.016%, 전부 fail-closed)을 실측한 기록. 그 수치의 재현 도구가 이것뿐이다 | **P21 재발 후보로 지목돼 등재됐다**(2026-09-01 구조 조사): production import 0 · 실험 import 0 · 테스트 import 1(`test_cg_mrs_reader.py:18`) — `LEGACY_REGISTER` 서두가 적은 실패 모드("테스트가 있었고 아무도 부르지 않았다")와 **문자적으로 일치**하는 유일한 모듈이다. 게다가 `HANDOFF.md:125` 가 이 파일을 stage2 "실행 기제"로 지목하는데 **그 사슬의 어떤 파일도 import 하지 않는다** — 다음 세션이 `handoff_reachability.py` 때와 똑같이 "배선돼 있다"고 읽을 표면이었다. **삭제하지 않는 이유**: `LEGACY_REGISTER` 규칙 3(후계자를 못 적으면 legacy 가 아니라 미결정)을 만족시킬 `superseded_by` 가 없다 — 정직한 status 는 실측 재현 도구다 |
