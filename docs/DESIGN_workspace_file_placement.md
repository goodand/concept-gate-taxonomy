...
**"이미 결정돼 있는가"는 파일명 기반 lexical 검색으로 답할 수 없다.** 결정
문서의 **경로**가 질문의 어휘를 포함하지 않을 수 있기 때문이다. 이번 사례의
감사 문서 경로는 `notes/audits/vault/symlink-vs-moc-2026-07-30.md`였고, 내
검색은 "디렉토리 정리 / 파일 배치 / DESIGN_DECISION / canonical"을 **파일명
기준**으로 훑었다. 경로 교집합은 0이다 — **단, 본문 교집합은 0이 아니다**
(`canonical`이 본문에 8회 등장, 2026-08-02 재확인). 즉 실패 지점은 "검색
어휘가 안 맞았다"가 아니라 "검색 **범위**가 파일명에 국한됐다"였다 — 더
정밀한 재현은 `docs/H1A_PROBLEM_ANALYSIS.md` §4 W-B.
