...
## 후속 — 이 라운드가 삭제 대신 만든 것

사용자 지적으로 방향을 바꿨다: **"삭제 하거나 legacy file 취급을 하거나".**
이 저장소의 실제 피해는 디스크가 아니라 **잘못된 재사용**이므로(P21의
`handoff_reachability.py`), 삭제 대신 [[LEGACY_REGISTER]]를 만들었다 —
표기·후계자·복구 명령을 한 곳에 모으고 `test_legacy_register.py`가 그 등록부의
거짓말을 막는다.

그리고 내 3라운드 기준이 과보수적이었음을 인정한다: **origin에 push된 것의
삭제는 되돌릴 수 있다.** 그 사실을 넣으면 완전 push된 worktree 4개(190M)와
재생성 가능한 인덱스(92M)가 실제 후보다 — 등록부 §"삭제 후보"에 복구 명령과
함께 적었고, 승인 대기 상태다.
