...
> ## 🔴 최신 상태는 §I다 (2026-08-05)
>
> **Q11·Q12 판정 모두 도착·반입.** Q12=F(typed-scope split)로 H1a는
> 계속되나 D-H1a-12 §16의 freeze 해제 조건 12개가 전부 미구현이다.
> §H(Q10 시점)는 이미 지난 세대다 — §H.5가 요구한 R1·Q10.2·Q11 확정·
> 독립 리뷰는 **전부 완료됐지만, Q12가 그 위에 새 조건을 얹었다.**
> §I가 현재의 게이트 목록이다. 아래 §0 표는 아직 Q10 시점 서술이니
> §I를 우선 신뢰하라(같은 이유로 §H 서두도 이미 한 번 과거 시점이었다 —
> "해결됨 표시는 그 시점의 평가"라는 함정이 이 문서에서 반복되는 걸 막으려면
> 배너 자체를 최신으로 유지하는 수밖에 없다는 뜻이지 다음 세션이 또 하단부터
> 읽어도 된다는 뜻은 아니다).
...
## I. Q11 전체 적용 + Q12 반입, 가드 게이트가 assert_9를 재발견 (2026-08-05)

§H.5의 게이트 5개(R1·Q10.2·§H.6 Q11 확정·독립 리뷰·R2) 중 **R2(양 arm 재실행)만
빼고 전부 완료됐다.** 그런데 완료 도중 독립 리뷰 3명 전원이
`FREEZE_BLOCKED`를 냈고(`docs/feedback/h1a_repair_review_20260804.md`),
그 설계급 발견이 새 판정 요청(Q12)으로 이어졌다. 그래서 R2는 아직도
실행되지 않았다 — 막힌 이유가 Q10 시절과 다를 뿐이다.

### I.1 완료된 것

- **Q11 전체 적용** — `DESIGN_DECISION_H1a_allowed_rendering.md`(Q11=D,
  Q11.1=A, Q11.2=A). 공통 기본허용 규칙을 양 arm에 byte-identical 방출,
  REMOVED에 축별 허용문 없음.
- **리뷰가 잡은 결함 3건 전부 수정** — 공허했던 `assert_5`(실제 템플릿
  바이트로 발동 재현 테스트로 교체), `_h1a_cohort.py::freeze()`의 무조건
  덮어쓰기(`CohortOverwriteRefused`), 정책 계층 미배선(`build_cohort()`에
  배선).
- **가드 음성-테스트 커버리지 게이트 신설** —
  `test_guard_negative_coverage.py`(repo root, AST 기반). 이 게이트를 이
  브랜치에서 실행하자 **사전 정보 없이**
  `assert_9_default_permission_is_byte_identical_across_arms`를 독립
  재발견했다(raise 경로 3개 전부 코드 구조상 도달 불가 — 렌더러가 비교
  상수를 자기 자신에게서 직접 방출). 모킹으로 날조하지 않고
  `KNOWN_UNPROVEN`에 등재.
- **Q12 반입** — `DESIGN_DECISION_H1a_identification_validity.md`(Q12=F,
  typed-scope split). `outside_domain_knowledge`(양 arm 금지 유지)와
  `source_meta_reasoning`(KEPT만 Q1로 금지)을 형제 정책 범주로 분리해
  표적 대비를 복구한다. 요청서(`correspondence/DESIGN_REQUEST_H1a_identification_validity.md`)는
...
### I.6 (2026-08-17) D-H1a-16/17 + **freeze gate 개방, 40 trial 동결**

**D-H1a-16**(Q16=A, 정본 확장) → **D-H1a-17**(Q17=B)이 그 적용 범위를 한정.
핵심 구분: *"그 의미가 인과적으로 중요하다"와 "그 의미를 반드시 한 특정 기계
검증 경로가 인증해야 한다"는 같은 명제가 아니다.*
`target-critical` 단일 enum을 `causal_semantic_critical` /
`canonical_audit_critical`로 분리했고 후자는 현재 0개다(우연적 — 독립 경로가
사라지면 다시 채워야 함).

**`INDEPENDENT_SEMANTIC_REVIEW_PASSED = True`**(`73b6682`), typed-scope 코호트
manifest 동결(`9e14316`, 40 trial, `H1AT` 접두사). **trial 미실행 — 승인 대기.**

**패턴으로 등재할 것 3건**:

1. ***요청서의 프레이밍이 판정의 범위를 정한다.*** Q16 요청서가 "어떻게 메울
   것인가"만 묻고 "그게 인과 추론에 필요한가 감사 완결성에 필요한가"를 안
   물었다. 판정은 물은 것에 답했고, 상위 목적을 재확인한 뒤 Q17로 다시 물어야
   했다. **요청서를 쓸 때 "이 질문이 상위 목적에 걸려 있는가"를 먼저 점검**할
   것.
2. ***검증 장치의 한계효용을 측정하라.*** 이번 세션 검증 장치 2,678줄 신설
   (실험 폴더의 29%) 대비 인과 추론용 trial 산출 0건, 12일 차단. 그 비율이
   Q17 상신의 근거가 됐다. 검증을 늘릴 때마다 **"이 검사를 빼면 잘못된 인과
   결론이 통과하는가"**를 물을 것 — D-H1a-17이 그 기준을 정식화했다.
3. ***반사실 주장은 실측 가능하므로 실측하라.*** D-H1a-17의 분류를 말로 받지
   않고 반사실을 돌렸더니 **첫 측정이 반증처럼 보였다** — 렌더 *후* 문자열
   변조는 compiler만 잡는다. 추적하니 위협 모델이 달랐고(정본 드리프트는
   `assert_9`, 렌더 후 변조는 manifest 해시), 두 실패 모드에 각각 독립 경로가
   있음이 확인돼 판정이 **강화**됐다. 반증처럼 보이는 측정을 끝까지 따라가라.

**자기 점검에서 찾은 내 코드 결함 2건**(사용자가 승인 조건으로 지시):
①`rendered_prompt_sha256` **필드 존재만** 단언하던 테스트 — 작동하는 해시와
장식용 필드를 구별 못 하는 형태(이 저장소가 반복해 당한 실패 모드).
②compiler의 `"allowed_by_default"` 문자열이 정책 상수와 중복 — **제거 불가**
(compiler는 정책 import가 AST로 금지), 그래서 audit 모듈에 일치 테스트를 둬
발산을 탐지 가능하게 했다. Q17.2의 b′를 구현하면 load-bearing이 된다.

상세는 `OPERATIONS_LOG.md` §15·§16.

### I.7 (2026-08-22) 실행 직전 결함 5건 + **40 trial 실행·채점**

실행 승인을 받은 뒤에도 바로 던지지 않고 하네스를 재실측했다. 전부 **실행
후에는 정당하게 고칠 수 없는** 항목이었다.

| # | 결함 | 실행 후엔 왜 못 고치나 |
|---|---|---|
| 1 | `_h1a_score.py`가 보존 코호트 경로에 하드와이어 — 두 번째 코호트는 채점할 곳이 없음. 자기 docstring이 "that wiring is not done"이라 적어둔 미완 배선 | 관측이 디스크에 있는 상태에서 덮어쓰기 거부에 막히고, 그 순간 가장 싸 보이는 수리는 **보존 아티팩트를 지키는 그 가드를 지우는 것** |
| 2 | 사전등록 §7이 은퇴 상수(`floor_or_ceiling_failure`, 08-16 은퇴)로 보고하라 지시 | §7 마지막 줄이 "결과를 본 뒤 코딩 규칙을 바꾸지 않는다"고 금지 |
| 3 | §8이 08-06 FREEZE_BLOCKED에서 끝나 게이트가 아직 막힌 것으로 읽힘, §3 조건표도 ⬜ | (오도) |
| 4 | `build_cohort()`이 `assert_freezable()` 반환값을 버려 §7의 licensed-path 기록 의무에 **구현이 없음** | 같은 이유 |
| 5 | 확증 코호트 raw에 **provenance 계약이 아예 없음** — 진단 쪽은 08-16부터 있었다 | 관측이 전송·모델에 대해 **증명 불가**로 고정 — QF-SELECT 5건을 재실행하게 만든 그 결함 |

5번이 새 패턴의 3회차다: **정책 층이 존재하지만 필요한 두 경로 중 한쪽에만
적용됐다.** 1회차는 미호출(08-06), 2회차는 호출했으나 결과 폐기(4번). 3회차가
가장 위험한데, 정확한 구현이 저장소에 **이미 있어서** grep이 부재를 못 찾고
함수도 있고 테스트도 통과하기 때문이다.

**적대 검토(haiku 4축, 사용자 지시)** — 비가역성 담당은 blocker 0으로 GO.
나머지 finding 중 **2건은 재실측으로 반증**했다:
- "동결 프롬프트가 렌더러 출력과 다를 수 있다" → 양 arm 바이트 동일
  (`6d945f4e…`/`856c3124…`, 3530/3447자). 리뷰어는 *고정하는 테스트가 없다*는
  점에서 옳고 *깨졌다*는 점에서 틀렸다 → 테스트 신설로 그 차이를 닫음.
- CRITICAL로 보고된 "덮어쓰기 거부 제거가 안 잡힌다" → 뮤테이션 직접 주입 시
  테스트 2건 FAIL. **리뷰어 하네스가 baseline에서 이미 깨져** 새 실패를
  귀속시킬 수 없었던 것. → 신규 패턴 P12(적대적 리뷰어의 하네스도 검증 대상).

**손코딩 대신 재조합(사용자 지시)** — 리뷰어들의 구조적 지적은
`transport: schema_forced_structured_output`이 검증 불가능한 주장이라는 것.
E2.4가 이미 해결해 뒀고 docstring에 근거까지 있었다("전송의 강제는 아티팩트에
보이지 않으므로, N건 전부 적합하다는 주장은 재실행 가능한 것에 근거해야
한다"). `_cohort.py::schema_errors`를 **verbatim 복사**해 raw 조립 경로에
배선. import가 아니라 복사인 이유는 D-H1a-1(E2.4 동결 구현 사후 변경 금지) +
`E2.4_ISSUE_REGISTER` [DONE] #6(한 실험이 다른 실험 모듈을 조용히 실행).
`inspect.getsource` 대조 테스트로 고정 — **핀 없는 사본은 결국 서로 다르게
판정하는 검증기 두 개**다.
...
40/40, 전송 실패 0, 완주 20/20, Stage A 통과, 스키마 적합 40/40 위반 0.
보존 코호트는 0/20 대 **0/20**이었다. 병기는 D-H1a-18 Q18.2=c의 조건 아래에서만
허용되며, **"재설계가 겨눴고 REMOVED가 0→2로 움직였다"는 이 절의 최초 문장은
철회됐다**(조건 3 위반). **ceiling 방향은 `unknown`**이므로
(QF-DEFER `material_unavailable`) 2/20을 포화 배제로 읽을 수 없다.
보고 정본은 사전등록 **§9**.

**trial_id 매핑을 증명했다.** 08-03 기록이 이미 "`journal.jsonl`은 content
hash 키라 신원 복구에 못 쓴다"고 경고했고 이번에도 그랬다. 새 실패는 다른
층 — 스크립트는 신원을 반환값에 실었고 정상 작동했지만 **반환값이 통지
채널에서 잘렸다**. 해법: agent 호출을 한 글자도 안 건드리고 후처리만 지문
맵으로 바꿔 `resumeFromRunId` 재생(**0 토큰, 75ms** = trial 재실행 0),
journal 40개 전문과 지문 대조 → **전단사 확인**(서명 40/40 구별). 발송 순서
추론은 정황이고 이것은 내용 기반 증명이다.

**자기 결함 1건 — 실행이 내 테스트를 반증했다.** 누출 탐지 테스트가 "누출
없음"을 "그 파일이 존재하지 않음"으로 검사하고 있었다. 후자는 채점 순간
거짓이 됐다. **P1을 잡으려고 나흘 전에 쓴 테스트가 P1을 저질렀다.** 실행 전후
바이트 불변 비교로 교정(부재는 `None`으로 기록해 "전에도 후에도 없음"도 통과),
단언은 지우지 않았다.

게이트: H1a **365 passed / 1 skipped**, 루트 **8 passed / 0 failed / 1 blocked**.

**다음은 실행이 아니라 판정 상신이다** — 종결 보고 / QF-DEFER 재료 확보 /
상위 목적 이행 중 무엇을 할지는 estimand·범위 문제(사전등록 §9.7).

### I.2 남은 것 — 이 섹션에서 반복 서술하지 않는다

**§16의 freeze 해제 조건 12개.** 전문은 `DESIGN_DECISION_H1a_identification_validity.md`
§16, 실행 순서는 `docs/HANDOFF.md` §10.4(항목별로 펼쳐 뒀다 — 여기서
또 베끼지 않는다. 세 군데에 같은 목록을 유지하면 그중 하나만 갱신됐을 때
바로 이 문서 자신이 경고하는 함정이 재발한다).

**R2(양 arm 재실행)는 §16이 전부 구현된 뒤에만 실행한다.** 최초 40 trial과
병합하지 않는다(D-H1a-12 §15).

### 미커밋 / 미승인

| 항목 | 상태 |
|---|---|
| Q5~Q8 적용분 + Q9·Q10·Q11·Q12 반입분 + 40 trial 산출물 + 가드 게이트 커밋 | ✅ **완료.** `git log`로 재확인(2026-08-05) — `codex/h1-source-authority`에 전부 반영 |
| 두 브랜치 **푸시** | ✅ **완료.** `origin/codex/h1-source-authority`와 로컬이 일치(2026-08-05 재확인) |
| skills-catalog 승격 | ⬜ `methodology.md` 표 정정 + P3 2번째 에피소드. **신규 후보 2건**: (a) "리뷰 3회 통과 설계가 실행 후 식별 결함을 드러냄" (b) "가드가 precision을 위해 의도적으로 완화된 지점이 정확히 결함 지점이 됨" — 2026-08-05에 별건으로 승격된 것(가드 공허/자기참조 사례, worktree 손복사)과는 다른 후보라 아직 미승격 |
| E2.4 브랜치에서 H1a 제거 | ⬜ **선택**(W3) — 사용자 결정 대기, 상태 아님 |
