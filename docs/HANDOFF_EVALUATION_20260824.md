...
## 12. 2차 복원 시험 (같은 날, D-35/Q36 이후 상태) — 10/10, 그리고 **중의성 1건**

문항을 10개로 늘리고(1차 3문항 · 이번 라운드 앞선 시험 9문항) "회신을 기다리지
않고 지금 할 수 있는 일"·"낡은 것"을 추가해 물었다.

| 집계 | 결과 |
|---|---|
| 복원 | **10/10** 전건 근거와 함께 |
| 막힌 지점 | **0건** |
| 낡은 것 | **0건** — 끝난 일을 앞으로 할 일로 적어 둔 곳 없음 |
| 모순 | 1건 보고 → **중의성으로 판정** (아래) |

무맥락 agent가 §1의 요약("측정 도구는 완성됐고 그 도구가 정의할 수 없는 것
하나에 막혀 있다")과 병목(R2), 금지 3항, 회신 없이 할 수 있는 둘
(`sbn_spec.py` 확보 · role 198종 실사)을 전부 복원했다.

### 12.1 보고된 "모순"은 **내 문장의 중의성**이었다

지적: §2가 `stage2_fixture_manifest_v5.json`을 "**원문 0바이트** — 해시·locator만"
이라 적었는데 파일은 31KB이고 완전한 JSON이다.

실측: manifest entry의 필드는 `case_id`·`stratum`·`source_locator`·
`text_sha256`·`lf_sha256`·… 이고 **`text` 필드가 없다.** 즉 "원문 0바이트"는
**코퍼스 원문을 담지 않는다**는 뜻이고 사실이다(Q30.4의 sha256-only 규율).

그러나 **무맥락 독자가 "파일이 0바이트"로 읽었다.** 문장이 사실인 것과 읽히는
것은 다르다 — 그것이 이 시험의 목적이다. 수리했다: "**코퍼스 원문을 담지
않는다** — `text` 필드 없이 `text_sha256`·locator만. 파일 자체는 31KB다."

**1차·앞선 2차 시험은 이 문장을 통과시켰다.** 문항을 늘리고 "낡은 것"을 물은
이번 라운드가 잡았다 — 시험을 강화하면 새 결함이 나온다.

### 12.2 그래프 도달성 — 이번 구간 신규 8건 **고아 0건**

`Name`/`ANA` 사슬의 신규 문서 전건이 backlink ≥2다(D-35 판정문 3 · Q35 2 ·
Q36 2 · D-33-V 7 · Q34-B 4 · ADOPTION 원장 4 · 순환 README 3 · D-34 판정문 5).
"만들 때 연결하라"가 이번엔 지켜졌다 — 직전 라운드는 고아 1건이었다.
