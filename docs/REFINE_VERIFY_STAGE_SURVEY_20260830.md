...
계열 접두를 붙여야 한다. ~~(예: `D-I3` / `M-I3`)~~ **[2026-08-31 정정]** 이 하이픈
표기는 등록부 규약 확정 **전**의 제안이다. 정본은 `docs/IDENTIFIER_REGISTER.md:25`
의 콜론 형식 — `directive:I3` / `mechspec:I3`. (§14 참조. 두 표기가 공존하면
FQN 게이트가 하나만 인정할 때 다른 하나가 위반으로 남는다.)
...
2. `mechanism_spec` 의 feedback 클래스 9종을 우리 obligation 어휘와 대조한다 —
   ...
   **[2026-09-01 정정]** 이 수적 일치는 **소멸했다** — 레지스트리는 이제
   **11종**이다(`claim.evidence_provenance` · `graph.feature_type_consistency`
   가산). 따라서 "개수가 같다"에 기댄 대응 가설은 근거를 잃었고, 대조가
   필요하다면 개수가 아니라 **어휘 내용**으로 해야 한다. 이 진부한 수치를
   그대로 베껴 커밋 메시지에 "9종째"라고 적은 것이 적대검증에 잡혔다.
...
## 14. 불변식 통합 표의 대안 — 축 지도 (2026-08-31)

사용자: "'불변식 통합 표'를 꼭 만들 필요는 없지만 그것의 상위 목적 달성을 하기
위하여 대안이라도 있어야 해." 상위목적을 사다리로 다시 세우면 통합 표의 진짜
요구는 문서 정돈이 아니라 **판정의 지시 대상 확정**이다 — "Verify 가 I3 를
어겼다"가 어느 I3 인지 말할 수 있어야 한다.

### 14.1 실측 — 두 집합은 합칠 것이 아니라 다른 축이다

77쌍(mechspec I1~I7 × directive I1~I11) 본문 전문 교차 대조(haiku 위임,
lead 재실측으로 인용 확인):

```text
DISJOINT 76 · OVERLAP 1 · TENSION 0 · DERIVES 0
```

- **mechspec = 상태 계약 축** — 어떤 상태가 허용되는가 (모순 없음 · 지원/의무 ·
  verified-region 보호 · provenance · 최소 rollback · 유한 retry · abstention)
- **directive = 권한 경계 + 표현 체계 축** — 누가 무엇을 해도 되는가(I1~I7),
  판정 어휘·의무 형식·인증 무순환(I8~I11)

**유일한 겹침: `mechspec:I2 × directive:I9`.** 원문 재실측:

> mechspec `:286-289` — "Unsupported commitments are not allowed to silently
> persist as if already justified."
> directive `:310·:350-355` — UNKNOWN(근거 부족)을 `canonicalized → PASS` 처럼
> 바꿔서는 안 된다.

같은 금지("근거 없는 것을 근거 있는 것처럼 위장하지 마라")를 상태 축과 정규화
축이 **독립적으로** 발행한 것이다 — 축이 교차한다는 뜻이 아니라 이 금지가 시스템의
하중 지지 불변식이라는 증거다. 2026-08-31 에 우리가 두 번 저지른 결함(잔여의
PROSE 낙수 세탁 · skip 의 PASS 세탁)이 정확히 이 지점이다.
...
A 의 범위를 재다가 나왔다: `qa_v7.py:495` 가 `R.check("I1 …")` 형식으로 자체
I1~I7 을 **발행**한다(10건). directive·mechspec 에 이어 **셋째**이고 등록부에
없었다 → `I@h1a-scope` `EXTERNAL` 행으로 등재(충돌 서술은 뜻 열에).

**게이트 한계의 실증**: `test_no_unregistered_issuance_by_an_unowned_letter` 는
**소유자 없는 글자**만 잡는다. `I` 는 소유자가 넷이라 다섯째 발행이 조용히
지나갔다 — **충돌 탐지는 소유자-부재 탐지와 다른 검사다.** 잡은 것은 게이트가
아니라 A 범위 실측이었다.
