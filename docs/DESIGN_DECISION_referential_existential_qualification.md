...
| 표면 유형 | 건수 | 무엇을 뜻하는가 |
|---|---:|---|
| 양화 한정사 | **395** (2%) | 결박자가 **진짜 양화력**을 갖는다 → 결박자를 일괄 제거하면 실제 scope 연산자를 지운다 |
| 인칭 대명사 | **7,430** (47%) | 결박자가 있으나 양화 표지가 없다 |
| 지시사 | **271** (2%) | 같음 |
| 그 외 | 7,714 (49%) | 관측 층에서도 미분류 |
...
## B.2 우리 문서의 세 번째 오해 소지 — Q34-B §2의 "96% 미결정"

Q34-B는 닫힌 목록을 **양화 어휘로만** 잡아 "미결정 96%"라고 적었다. 그것은
그 목록에 대해서는 참이지만, **"닫힌 목록으로는 처리되지 않는다"는 함의를
과장한다** — 대명사·지시사 목록을 더하면 관측 층에서 **51%가 식별**된다
(양화 2% + 대명사·지시사 49%).

정확한 서술은 이것이다:

> 닫힌 목록은 **관측 층에서** 후보의 51%를 유형화한다. 그러나 그 유형이
> 의미론적 경계와 일치하는지는 **바로 미정인 것**이고(판정 §4의
> `representation → semantics` 금지), 나머지 49%는 관측 층에서도 미분류다.

**이것을 정정으로 기록하는 이유**: "닫힌 목록으로 2%만 처리된다"는 우리
문장이 판정 재료로 쓰였다. 방향(경계를 목록으로 못 그린다)은 유지되지만
**수치의 함의가 달랐다.** 이 세션에서 같은 형태의 정정이 세 번째다
(Q33 §1 → Q34-B, Q34-B §2 → 여기).
...
- B.1의 세 닫힌 목록(양화 24종·인칭 대명사 19종·지시사 4종)은 **우리가 고른
  것**이다. 목록을 넓히면 51%가 변한다 — 그 선택이 경계 결정이 되지 않도록
  목록을 본문에 전부 실었다.
- 대명사·지시사를 "referential"이라 부르지 **않았다**. 표면 토큰이 닫힌
  목록에 있다는 관측만 적었다. 그 의미론적 지위가 바로 미정이다.
