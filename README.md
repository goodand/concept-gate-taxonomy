...
├── conceptgate/           ← 정본 소스 패키지 (사본 없음)
│   ├── concept_gate_v7.py ← v7 코어 (stdlib만 의존)
│   ├── cg_partwhole.py    ← part-whole 어댑터 (vendor/obo-relations 조립)
│   ├── cg_gufo.py         ← Scior/gUFO rule metadata adapter (fallback 포함)
│   ├── cg_input_linter.py ← MCP 입력 JSON 사전 점검 linter
│   ├── cg_normalizer.py   ← evidence-carrying 경계 어댑터 (단계 파이프라인)
│   ├── cg_owl.py          ← OWL 2 DL 직렬화 + HermiT 분류 (Java 필요)
│   ├── cg_graph_export.py ← GraphExporter: JSON/Mermaid/GraphML
│   └── server.py          ← FastMCP 어댑터 (`python -m conceptgate.server`)
├── qa_v7.py               ← v7 QA 스크립트 (101개 검증) ★ 최신
├── test_server.py         ← MCP 서버 테스트 (66개)
├── qa_v6_3.py             ← v6.3 QA (33개, 회귀 참고용)
├── concept_gate_v6_3.py   ← v6.3 소스 (회귀 참고용)
├── Dockerfile             ← 배포 이미지 (JRE 포함 — HermiT가 Java를 요구)
├── vendor/
...
# 소스 자체 인라인 테스트 (60개)
python3 -m conceptgate.concept_gate_v7

# MCP 서버 테스트 (66개, fastmcp 필요 — normalizer surface 포함)
python3 test_server.py

# normalizer 단위 테스트 (16개)
python3 -m pytest test_cg_normalizer.py -q

# gUFO 정합 회귀 (8개)
python3 -m pytest test_semantic_regressions.py -q

# GraphExporter 인라인 테스트 (12개)
python3 -m conceptgate.cg_graph_export
...
[`docs/LOCAL_INSTALL_GUIDE.md`](docs/LOCAL_INSTALL_GUIDE.md)를 참고하세요.
...
### Normalizer surface (자연어 → evidence-carrying concepts)

`cg_normalizer`는 자연어를 concept-gate 입력 JSON으로 바꾸는 **경계 어댑터**입니다.
서버는 LLM을 호출하지 않습니다 — agent가 sense/feature/관계를 **제안**하고,
normalizer는 확인 가능한 조건만 결정론적으로 **판정**합니다.

입력층은 단계로 분리되어 있어, 실패 시 어느 단계의 책임인지 즉시 드러납니다:

```text
make_snapshot → lookup_senses → (agent 다의어 해소) → assemble_concepts → run_pipeline
   snapshot        lookup          selection·crosswalk       assemble·lint       gate
```

- `normalizer://protocol/v1` (resource) — agent가 따라야 할 단계와 금지 사항.
- `normalizer://relations/v1` (resource) — 이론 어휘(Winston 1987 / gUFO) →
  운영 `relation_hint` crosswalk. `mapping_status`가 `exact`가 아닌 항목
  (conditional/unmapped)은 조건 확인 또는 거부가 필요합니다. `feature_activity`는
  대응 어휘가 없어 **의도적으로 unmapped**입니다(침묵 매핑 금지).
- `make_snapshot` (tool) — 원문을 NFC+sha256으로 고정. 이후 인용 span의 기준.
- `lookup_senses` (tool) — 표면형의 sense 후보 조회. `out_of_inventory`면
  `local:` namespace 임시 sense를 만듭니다.
- `assemble_concepts` (tool) — 제안 묶음을 검증·조립해 lint 통과한 concepts JSON을
  반환. 각 claim에 span·source hash 기반 `verification_status`가 붙습니다
  (`confidence`는 검증이 아닙니다). 실패 시 `stage` 필드가 원인 단계를 가리킵니다.

명명 규칙: 현재 산출물은 evidence⊨claim을 기계적으로 증명하지 않으므로
**evidence-carrying feature JSON**입니다(proof-carrying 아님). `verification_status`
사다리는 L0 스키마 → L1 span·hash 검증 → L2 관계 제약 → L3 entailment 순입니다.
...
- MCP 서버 테스트: 66건 (`python3 test_server.py`)
