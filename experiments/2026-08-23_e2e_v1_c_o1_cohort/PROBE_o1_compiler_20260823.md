...
## 추기 (같은 날) — 프로브 A 해소: 레지스트리가 세션 중 갱신됐다

정의 생성 후 레지스트리가 이 세션 안에서 `o1-compiler`를 인식했다 — 이월
사유(세션 재시작 필요)가 소멸했으므로 즉시 재실행했다.

- **dispatch**: `agentType: o1-compiler` (진짜 레지스트리 경유), haiku,
  1,711 토큰 — 정의가 의도한 만큼 얇다.
- **입력**: 발명 다중 양화 문장 `Some tikk praxes every zorble.` (scope
  선택까지 관찰하는 형태)
- **산출**: `∃x tikk(x) [∀y zorble(y) [praxes(x,y)]]` — 표면 어순 scope.
  **커널 판정: 오류 0, 닫힘. 스키마 판정(준비물 ②, v0 O1 profile): VALID.**
  다중 양화 중첩·restriction 형태·항 규율 전부 정확.
- **출력 규율: fence 위반 재현** — 정의 경유에서도 ```json fence로 감쌌다.
  프로브 B와 일관: **위반은 인라인 방식의 산물이 아니라 프롬프트 규율
  일반의 한계다.** schema-forced dispatch 필수 판정이 이중 실측으로 굳었다.

준비물 ①·②의 상호 검증이 여기서 닫힌다: ①의 subject가 낸 실물 IR을 ②의
스키마가 수용하고, ②가 거부하도록 설계된 것(fence 문자열, 수식 인자,
restriction 부재)은 커널과 같은 경계에서 거부한다.
