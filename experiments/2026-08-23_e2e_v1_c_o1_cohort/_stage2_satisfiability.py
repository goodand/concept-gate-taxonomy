"""MEASUREMENT_SATISFIABILITY_V1 — 측정 가능성 게이트 (D-E2E-v1-25 Q25.4).
...
signature를 subject 방언으로 재렌더한 것. 렌더 규칙은 단순하다 — desugar가
만든 `forall(v, True, implies(L, R))`만 제한식 형태 `forall(v, L, R)`로
되돌리고, 그 밖의 위치에 남는 implies/or 등 방언 밖 연산자는 렌더 불가
(= subject_dialect_expressible 실패)로 정직하게 보고한다. 이것은 oracle
...
GATE_ID = "MEASUREMENT_SATISFIABILITY_V1"

# 동결 subject 방언 = O1_V1 constructor 5종 (freeze_stage2가 정본)
from freeze_stage2 import O1_V1_CONSTRUCTORS  # noqa: E402

_SCHEMA = formula_json_schema(tuple(O1_V1_CONSTRUCTORS))
...
def render_witness(signature: dict) -> dict:
    ...
    def walk(node: Any) -> Any:
        ...
        if kind in ("forall", "exists"):
            body = node["body"]
            if (kind == "forall" and _is_true(node["restriction"])
                    and isinstance(body, dict) and body.get("kind") == "implies"):
                return {"kind": "forall", "var": node["var"],
                        "restriction": walk(body["left"]),
                        "body": walk(body["right"])}
