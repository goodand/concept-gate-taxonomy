...
> **Provenance: reconstructed record.** `trials.json`은 이전 세션 대화 컨텍스트에서
> 복원되었으며, 원 실행의 모델·시각·context-isolation 조건·원문 응답을 입증하는
> 독립 로그가 현재 저장소에 없다. 아래 수치는 복원 데이터에 대한 scorer 재계산값이지
> 검증된 empirical trial 결과가 아니다.
...
- **record**: 이전 세션 컨텍스트에서 복원된 13개 응답. 실행 provenance가 없어
  콜드 Haiku trial 여부와 cell당 독립성은 현재 증명할 수 없다.
...
다음 해석은 복원 기록 안에서만 성립하며 empirical evidence로 사용하지 않는다.
...
- **실행 provenance 부재**: 모델, temperature, 실행 시각, 콜드 컨텍스트 독립성,
  도구 접근 0 여부를 입증할 원 로그가 없다.
...
`trials.json`은 이전 세션 컨텍스트에서 복원된 기록이다. fixture.json은 입력+oracle만
저장하고 canonical 응답은 evaluate가 생성한다(이중 저장 drift 방지). 새 empirical
trial은 E2.1에서 실행 provenance와 원문 응답을 함께 저장한다.
