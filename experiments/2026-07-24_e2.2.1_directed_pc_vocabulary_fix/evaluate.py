"""E2.2.1 scorer: single-arm control-affordance check for the directed PC.

NOT a B-C hypothesis test (E2.2's main hypothesis is not reused here). The
only question: with structural_composition (and the full FeatureType
vocabulary) explicitly exposed in prompt+schema, does the directed positive
control (dir1_directed) now pass at an acceptable rate? E2.2 measured 0/10
under a prompt/schema that never mentioned this vocabulary at all.

Go/No-go: single criterion, pass_rate >= 0.80 (same threshold E2.2 used for
its c6_directed_pc_passes criterion, kept for comparability).

`_parse_ok` and `classify_directed_repair` below are copied verbatim from the
frozen E2.2 scorer (experiments/2026-07-23_isa_certificate_structure_bvsc/
evaluate.py, commit 49f030b) so the pass/fail definition is identical to what
E2.2 measured 0/10 against — only the prompt/schema under test changed. E2.2
is NOT modified.

Run (repo root or worktree):
    python3 experiments/2026-07-24_e2.2.1_directed_pc_vocabulary_fix/evaluate.py
"""
...
# ── copied verbatim from E2.2 evaluate.py (commit 49f030b) ─────────────────
