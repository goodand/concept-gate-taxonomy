...
> ⚠️ **2026-08-22 정정 (D-H1a-18 Q18.2=c).** 아래 취소선은 판정 수령 전에 쓴
> 것이고 조건 3에 걸린다. 원문은 철회 이력으로 보존한다.
>
...
> 움직인 것은 그 진단이 옳았다는 방향의 관측이다.~~

D-H1a-18이 제시한 허용 표현을 그대로 쓴다:
...
08-03 세션이 0/40을 "조작 무효"로 읽지 않고 Q10을 상신했다는 이력은 사실
기록으로 남긴다 — 그 이력과 §4의 관측을 인과적으로 연결하지 않는 것이 조건 3이다.
두 코호트의 지위는 다르게 유지한다(조건 5): 08-03은
`completed_nonidentifying`, 08-22는 repaired-surface observation.
