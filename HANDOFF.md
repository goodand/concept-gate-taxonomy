...
next_action_code: PROPAGATION_AUDIT__OR_ACQUIRE_SBN_SPEC_OR_REACQUIRE_PMB   # **열린 재검토 1건 닫음** — `H1A_PROBLEM_ANALYSIS` §23. 우리 Q36 문장 '다른 주석 corpus는 R1·R4를 채우고 R2에서 막힌다'는 **양쪽 절반이 같은 가정에서 무너졌다**: R2 절반은 **D-36 §6이 이미 정정했는데 패턴 원장에 6일간 전파되지 않았고**(§20.3이 더 센 형태 '실패한다'를 보유), R4 절반은 D-37 아래 **`FAIL`이 아니라 `BLOCKED`**(M 부재로 평가 불가). 공통 가정: **'두 개를 가지면 요건이 자동 충족된다'** — 두 번째 source는 후보일 뿐 채움은 매번 별도 입증이다. 신규 G161~G163 · **P26 신설**(정정이 정본까지 도달하지 않는다). (1) **1순위 — 전파 감사**: D-19~D-37이 `> 우리 문장` 형태로 인용해 정정한 블록을 전부 뽑아 저장소에서 정정 마커 없이 남은 원형을 찾는다. §23.6이 자백한 대로 이번엔 D-36 §6 하나만 따라갔다. 게이트 후보는 §23.5에 기록(미구현). (2) **부재 판정 격하 유지** — D-37 정본 §4 '대칭 결함 0건'은 grep 범위 미검출이지 부재 증명이 아니다(backlink 순회 미실시). (3) 회신 없이 가능: `sbn_spec.py` **원천** 확보 · role 어휘 **198종 미검토** 실사(`EQU` 12,501) · **`pmb_gold` 재취득**(12,053건 소실, 2026-08-29). (4) **보류 중인 질문** — 사용자 지시로 미상신: 'L2 사용 기록이 R2 후보가 되는가'. D-37이 R2⊥R4를 확정했으므로 깨끗이 분리해 물을 수 있다. 금지 유지: `operational_patch`·`immediate_projection`·V3 제작·`ANA` 배제 되돌리기. **코호트 dispatch 누계 0건**
