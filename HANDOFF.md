...
next_action_code: REEXAMINE_R4_CLAIM__OR_ACQUIRE_SBN_SPEC_OR_REACQUIRE_PMB   # **프로토콜 4단계 전수 완료**(D-37: 설계→적대적 검증→검증→저장). 건너뛴 `설계 적대적 검증`을 사후 복구했고 그 결과 정본 §5·§6이 **정정**됐다 — '사슬 전체가 같은 첫 항을 가리킨다'는 내 일반화가 틀렸다(D-35 `:510`은 R2 방향). 결론(R4는 M의 하류)은 유지, 근거는 교체. 판정문 논리 주장 4건은 진리표로 **독립 재현**했다. (1) **열린 재검토** — 우리 Q36 주장 '다른 주석 corpus는 R1·R4를 채우고 R2에서 막힌다'(`DESIGN_REQUEST_independent_verifiability_constraint.md:59`)가 D-37의 **조인 R4** 아래서도 성립하는가. 항목만 열었고 닫지 않았다. (2) **부재 판정 격하** — 정본 §4의 '대칭 결함 0건'은 grep 범위 미검출이지 부재 증명이 아니다(backlink 순회 미실시). (3) 회신 없이 가능: `sbn_spec.py` **원천** 확보 · role 어휘 **198종 미검토** 실사(`EQU` 12,501) · **`pmb_gold` 재취득**(12,053건 소실, 2026-08-29). (4) **보류 중인 질문** — 사용자 지시로 미상신: 'L2 사용 기록이 R2 후보가 되는가'. D-37이 R2⊥R4를 확정했으므로 깨끗이 분리해 물을 수 있다. 금지 유지: `operational_patch`·`immediate_projection`·V3 제작·`ANA` 배제 되돌리기. **코호트 dispatch 누계 0건**
