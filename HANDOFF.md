...
next_action_code: MERGE_INVARIANT_TABLES__THEN_TWO_PASS   # **두 계열 연결 완료(2026-08-30)** — SURVEY §12. mechanism_spec ↔ DIRECTIVE ↔ obligation_layer_roadmap 세 결절점에 양방향 저자 간선을 넣고 도구로 검증(색인 간선 +11 · vault_search 2위 · backlink 0→2). 관계는 `supersedes` 가 아니라 **`complements`** — 공유 어휘는 obligation·candidate·provenance 셋뿐, 나머지 상보(실측). **wikilink 위생 게이트 신설**(`test_wikilink_graph.py` 43 · `scripts/wikilink_graph.py`, 커밋 9552799) — 죽은 링크·AMBIGUOUS·EXTERNAL 을 가른다. 실제 결함 2건 수리. (0) **식별자 이름공간은 로그만 남겼다** — [[H1A_PROBLEM_ANALYSIS]] §24 (**G164**). 회고 안 중복 정의 **0건**(내 첫 측정 48건은 재인용을 정의로 센 P25 재발, 실물로 정정). 충돌은 문서군 **사이**다 — **프로토콜 사후 적용(§24.6)이 표를 정정**: P 3중 · W 2중 · **I 3중**(evidence-evaluator 회고가 I186~I231 을 이슈 번호로 씀 — 우리 G 와 같은 역할) · **M 4중**(회고 방법/roadmap 마일스톤/mechanism_spec/D-19 능력 축). G164 는 표 행으로 재발행. **고치지 않은 이유**: 셋 중 둘이 우리 소유가 아니다(DIRECTIVE 는 verbatim, vault 등급은 도구 소관). 처방은 재번호가 아니라 **인용 규약**이고 §24.5 에 외부 지적 대응 4문답을 적어 뒀다. **식별자 사전 신설**(2026-08-31, SURVEY §13) — `docs/IDENTIFIER_REGISTER.md` + `test_identifier_register.py`(24). A~Z 전수 → haiku 2축 문맥 판별 → **COLLIDES 10글자**(B C D F I M P R V W) · CITES_ONLY 4 · 단일 3. 키는 (글자,문서군) 쌍, 41행. 게이트 첫 실행이 등록부 행번호 22건 오류·표셀 `|` 불가·회고 G 형식 변천·P24~26 표 밖 발행을 잡았다. **충돌은 없애지 않았다** — 처방은 인용 접두. **역방향(뜻→글자)도 담았다**(G165): 문제 7글자·검증 6·등급 5 로 흩어짐 — 순방향은 오독, 역방향은 **집계 불가**. 고빈도 정규화 제안은 실측으로 세 개념에서 역전(우리 소유가 더 큼, G166) — 후보는 `검증`(V 287 vs M 124) 하나, 미결정. `M@mechspec` 은 mermaid 노드 ID 라 제외(M 3중). **인용 규약 = FQN `<문서군>:<글자><번호>`** 채택(2026-08-31, 회고 §24.8 — 재번호 143건·회고 규약 충돌 vs FQN 등록부 32→7, 원문 0건). 과거 인용 불변. **기호→영문→뜻 tree 는 30/36 에서만 성립**(G167) — `retro:G`(164개)는 한국어 '문제'에 붙인 **임의** 글자, 판정문 B·C·D·F 는 적대검증 **축 순서**. tree 는 initial 형에만 그렸고 `I→Invariant` 는 같은 단어·다른 내용이라 FQN 으로만 갈린다. **범위 정정(G168~G170)**: A~Z 전수가 `docs/` 한 폴더에만 돌았다. 광역 실측에서 `T` 는 진짜 누락 계열(ev-eval 코드), `H`·`A` 는 위양성(실험명·절번호), 첫 측정의 `E` 최다는 `E2.4` 를 `E2` 로 잡은 정규식 결함(P25 재발). **미등재 문서군 둘** — `experiments` md(1,932·19글자)·코드(809·15글자). 사전 적용 범위가 `docs/` 뿐이다. **동료 세션 조언 반영**(G171~G174): 등록부 밖 계열 **X·N·K·J** 등재(X 는 ev-eval 실패코드 대 h1a-scope mermaid 노드의 **저장소 간 충돌**), **세 번째 미등재 문서군** `h1a-scope` 추가, 위양성 유형 3종(`FP_DIAGRAM`/`FP_SECTION`/`FP_EXPERIMENT`) 신설, `미분류` 잔여 게이트. 내 뮤테이션 sweep '안 잡힘' 8건은 **전부 도구 결함**이었고 재시험 9/9 빨강. **상호 목격자 성립**(G175~G177): 동료가 `evidence-evaluator` 에 `FAILURE_CODES` 목격자를 걸었다(`8520567`, 글자+코드 고정). 내 삼중항 2곳 정정 — `D0` 누락 · `ev-eval:I` 는 **저장소 안 두 계열**(`contract.py` I1 대 회고 I136~231, 번호대가 안 겹쳐 사고 미발생). 게이트가 `(글자,문서군)` 중복을 잡아 **문서군을 저장소가 아니라 발행 단위로** 정정(`ev-eval` / `ev-eval-code`). **절반만 닫힘** — 역방향(우리 변경을 그쪽이 앎)은 그쪽 `SEMANTIC_BOUNDARY` 때문에 불가, 등록부에 명시. **식별자 분류기 구현 중**(G181~G184): (나)4 로컬 조사가 `scan_review_codes`(AST 다중노드+잔여계수)를 재사용 후보로 냈고 (나)5 github 는 30검색어 0건. 인라인 코드 스팬만 워크스페이스에 0건이라 새로 만든다. TDD 계약 25개 작성·적대검증·개정 완료, Sonnet 구현 대기. **미등재 범위 A~Z 처리 완료**(G178~G180): 발행 단위 13군 전수(정본 832·쌍 225). 발행 대 인용을 기계로 갈라 **소유자 없는 발행은 `H@experiments` 하나**(실험 이름, `FP_EXPERIMENT` 등재) — **글자 차원 닫힘**. 등록부는 **발행자와 충돌만** 담고 인용처는 안 담는다(§등록 범위). 옛 계약(모든 쌍 등재) 폐기 → `test_no_unregistered_issuance_by_an_unowned_letter` 로 이관. 문서군 9개 추가, `archive` 는 인벤토리만·권위 제외. 게이트 47/47. (1) **1순위 — 불변식 통합 표**: `directive:I1~I11` × `mechspec:I1~I7` 을 한 표로. 접두는 이제 등록부 형식 그대로: 양쪽 I1~I7/I11 을 한 표로 합치고 번호 충돌 해소(`D-I*`/`M-I*`). 연결은 했으나 **합치지는 않았다**. (2) mechanism_spec feedback 9종 대 `OBLIGATION_REGISTRY` 9종 대조. (3) 2-pass 실험(§8.4). (4) `vault_backlinks` 가 basename 사본을 오인한다(§12.4①) — 도구 소관 세션에 보고할 것. (5) `obligation_layer_roadmap` 은 h1-wt 만 고쳤다 — 나머지 4 worktree 는 머지로 받는다. 금지 유지: `operational_patch`·`immediate_projection`·V3 제작·`ANA` 배제 되돌리기. **코호트 dispatch 누계 0건**
...
| — | 2026-08-30 (여러 회) | **미기록** — 규약이 있었는데도 적지 않았다. 이 구간은 표로 좁힐 수 없고 `git log --since=2026-08-30` 로 따로 봐야 한다 |
| 2 | 2026-08-31 01:42 | `docs/H1A_PROBLEM_ANALYSIS.md` §24.13 (A~Z 미등재 범위 처리) → 같은 분에 `HANDOFF.md`·`docs/IDENTIFIER_REGISTER.md`, 01:43 커밋 `c8fdb06` |
| 3 | 2026-08-31 (직후) | `test_identifier_scan.py` 01:46 — 식별자 출현 분류기 TDD 계약 |
