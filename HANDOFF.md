...
next_action_code: TWO_PASS_REFINE_VERIFY__THEN_PROPAGATION_AUDIT   # **신규 1순위(2026-08-30)** — 최소 2-pass 실험. 근거: [[REFINE_VERIFY_STAGE_SURVEY_20260830]] §8. 다른 workspace 세션 판정으로 **Refine 의 상위 목적이 저장소에 처음 기록**됐다(= Verify 가 독립 검증할 수 있는 상태까지 구성하는 것. **Certified 가 Refine 의 종점이 아니다**). 그 판정이 v0 를 이렇게 못박았다: **'v0 가 수렴(5항)을 검증한다'고 쓰면 과장, '수렴을 향한 transition 을 관찰한다'가 정확**. 실측으로 우리 v0 는 **경우 A**(수리 후 재검증 없음, `test_e2e_v0_refine_verify.py:186` 의 `all_pass` 가 손작성)임을 확인했다. **작업**: `[6]` 손작성 verdict 를 걷고 `repaired` 에 `results_from_normalizer` 를 재실행해 `source.span_evidence` 가 UNKNOWN→PASS 로 바뀌는지 단언한다 → 경우 A→B 전이. 생산자는 이미 있다(`cg_obligations.py:327,332`) — 새 검사기 불필요. **loop scheduler·oscillation detector 는 아직 만들지 마라**(판정 권고: 아키텍처를 키우기 전에 2-pass 부터). 함께 등재된 미구현: `Refine success ≠ semantic correctness` 불변식 · provisional/unknown/unresolved **3층 분리**(현재 graph 쪽에 없음) · Kernel 은 **해석**을 canonicalize 할 권한 없음(I9 확장). (2) **전파 감사** — D-19~D-37 이 인용·정정한 블록 중 정정 마커 없이 남은 원형 찾기. (3) **감사 복구 항목** — SURVEY §6: 이 감사가 `설계 적대적 검증`을 건너뛰었고 그 대가로 축 A 가 이미 닫힌 W5 를 열린 것으로 보고했다. (4) 회신 없이 가능: `sbn_spec.py` 원천 · role 198종 실사 · **`pmb_gold` 재취득**(12,053건 소실). 금지 유지: `operational_patch`·`immediate_projection`·V3 제작·`ANA` 배제 되돌리기. **코호트 dispatch 누계 0건**
