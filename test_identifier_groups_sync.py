...
## 적대적 검증 (2026-08-31) — 5건 중 4건 기각

| 지적 | 판정 | 근거 |
|---|---|---|
| blocker 생성기에 9열 검사가 없다 | **기각** | `gen_identifier_groups.py:73` 에 있다. 검증자가 자기 scratchpad 에 만든 **가짜 생성기**를 쟀다 |
| major `stale` 검사가 게이트를 안 시험한다 | **기각** | `:159` 에 `pytest.raises` 가 있다. **내가 자체 검증에서 먼저 잡아 고친 것**을 옛 판으로 봤다 |
| major `sys.executable` 불일치 | **기각** | 게이트 하에서 재니 정확히 venv python 이다. 검증자가 **자기 셸**을 쟀다 |
| minor 테스트가 테스트를 import | **기각** | 테스트는 배포되지 않는다(`COPY conceptgate/` 만). 두 파서 대조는 **의도**다 |
| minor `docs` 마커 우회 | **이미 아래에 한계로 적혀 있다** |

**그러나 지적 1 밑에 깔린 위험은 진짜였다.** 표현은 틀렸지만 짚은 것은 남는다 —
**두 파서가 같은 방식으로 열 부족 행을 건너뛰면 등록부가 손상돼도 둘의 결과가
같아 아무도 울지 않는다.** 실측: 행을 8열로 줄이니 이 파일은 울었지만
`test_identifier_register.py` 는 **조용했다**(55 passed).

처방은 이 파일이 아니다 — **정본의 형식은 정본이 지켜야 한다.**
`test_identifier_register.py::test_every_series_row_has_the_full_column_count` 를
추가했다(음성 증명: 8열 손상 → 빨강). 파서의 일치는 파서끼리의 문제이고,
그것으로 정본의 건전성을 대신할 수 없다.
