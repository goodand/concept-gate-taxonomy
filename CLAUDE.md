...
**실물이 없으면 지어내서 재지 말고 잔여로 보고하게 한다.** 2026-08-31 에
검증자가 아직 없는 생성기를 **scratchpad 에 재구성해** 그것의 결함을 보고했다 —
그 측정은 재구성의 성질을 잴 뿐인데 받는 쪽은 실물의 성질로 읽는다(회고 G258).
같은 회신에서 **내가 이미 고친 판**을 옛 판으로 보고, `sys.executable` 은
**자기 셸**을 쟀다. 셋 다 "무엇을 쟀는가"가 어긋난 것이다.

**계약(구현 전 테스트)을 검증시킬 때는 한 문장을 더 넣는다**: *"구현은 아직 없다. **그 부재를 결함으로 보고하지 마라** — 공격 대상은 계약이다."* 2026-08-31 에 이것을 빠뜨려 지적 10건 중 **8건이 "구현이 없다"** 로 왔다(TDD 빨강을 결함으로 보고). 실질은 2건이었고, 검증자가 답하지 않은 질문 하나는 **내가 직접 실측**해야 했다 — 하네스가 형식은 강제하지만 **무엇을 볼지까지 강제하지는 않는다.**
...
## 도구를 만들 때 — 이것은 authoritative 인가 advisory 인가

새 도구·색인·요약을 만들 때 **먼저 묻는다.** 대답을 안 정하면 기본값이
authoritative 가 되고, 그것이 이 저장소에서 도구가 제거된 사유였다.

| | authoritative | advisory |
|---|---|---|
| 무엇인가 | **정확한 상태의 출처** | 재정렬·힌트 |
| 예 | 파일시스템 · git 이력 · 세션 기록 · 게이트 판정 | 색인 · 의미 검색 · 생성 MOC · 요약 |
| 할 수 있는 것 | 존재/부재를 **결정**한다 | 이미 나온 후보를 **다시 정렬**한다 |
| **못 하는 것** | — | **존재 여부를 결정하지 못한다. 낡았을 때 정확한 출처를 이기지 못한다** |

**근거는 우리 사고다.** `scripts/handoff_reachability.py` 는 제거됐다
(`docs/LEGACY_REGISTER.md:31`) — **"인덱스가 없으면 backlink 0건이라는 조용한
오답"** 을 냈기 때문이다. 색인(advisory)이 부재 판정(authoritative 의 일)을
하도록 놔둔 것이 원인이다. 워크스페이스 `CLAUDE.md` 의 권위 순서와
"생성 MOC 는 navigation artifact 이지 source authority 가 아니다"가 같은 말이다.

**적용례(2026-08-31).** `scripts/compaction_ledger.py` 는 세션 기록(authoritative)
에서 시점을 뽑고 **추측한 값을 넣지 않는다**(시간대를 못 읽으면 빈칸). 깨진 줄은
세어서 경고한다 — 부재와 미확인을 가른다.

**하네스와 규칙은 중복 규율이 다르다**(사용자 지시, 2026-08-31: "하네스는
최종산출물이 아니라 구현 단계에서 필요한 것"). **규칙·상수**는 한 프로젝트 안에
두 벌이면 반드시 갈라진다(G199·G213) — 단일 출처로 만든다. **하네스**는 구현
단계의 비계라 프로젝트마다 자기 것을 갖는 것이 정상이고, 다른 프로젝트의 하네스를
공유하려 하지 않는다. 가져오는 것은 **원칙과 어휘**이고, 그것은 우리 어휘로
우리 문서에 **한 번** 적는다.
